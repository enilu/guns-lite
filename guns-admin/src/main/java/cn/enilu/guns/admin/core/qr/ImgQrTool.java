...
public class ImgQrTool {
	...
	public static void createQrWithFontsAbove(QrImage para) {
		try {
			// 首先生成二维码图片
			BufferedImage qrImage = genBarcode(para.getQrContent(), para.getQrWidth(), para.getQrHeight(),
					para.getQrIconFilePath());
			...
			// 创建顶部文字的图片
			BufferedImage imageWithFonts = new BufferedImage(qrImageWidth, fontsImageHeight,
			...
			if (para.getWordContent().length() > qrImageWidth / para.getWordSize()) {// 每行限制的文字个数
				...
				fontsImageGraphics.drawString(para.getWordContent(),
						((qrImageWidth / para.getWordSize() - para.getWordContent().length()) / 2) * para.getWordSize()
								+ 20, // 总长度减去字长度除以2为向右偏移长度
						para.getWordSize());
			}

			// 从图片中读取RGB
			int[] ImageArrayFonts = new int[qrImageWidth * fontsImageHeight];
			ImageArrayFonts = imageWithFonts.getRGB(0, 0, qrImageWidth, fontsImageHeight, ImageArrayFonts, 0,
					qrImageWidth);
			...
			// 生成新图片
			BufferedImage ImageNew = new BufferedImage(qrImageWidth, qrImageHeight + fontsImageHeight,
	...
	public static void main(String[] args) throws IOException {
		for (int i = 1; i <= 1; i++) {
			QrImage para = new QrImage.Builder().setFileOutputPath("D:\\二维码\\test\\" + i + ".jpg")
					.setQrContent("http://www.baidu.com?a=" + "123").setQrHeight(300).setQrWidth(300)
					.setQrIconFilePath("D:\\二维码\\中间图标\\1.png").setTopWrodHeight(100).setWordContent("test" + 1)
